...
const uploadImageAndCreateRecipe = async () => {
  if (selectedImage.value) {
    ...
    try {
      ...
      //       // Now that you have the `autoGeneratedID`, you can update the user's document
      //       const user = auth.currentUser;
      // const userUID = user ? user.uid : null;
      ...
      if (userUID) {
        ...
        // Add the `autoGeneratedID` to the user's `posts` array
        if (userData && Array.isArray(userData.posts)) {
          userData.posts.push(autoGeneratedID);

          // Update the user's document with the modified data
